...
function run_case(c::Case, max_seconds::Float64)
    ...
    unsupported = setdiff(c.options, [:onepi])
    isempty(unsupported) || return (:skip, "unsupported options: $unsupported", 0.0)

    # Empty process (self-energy 1-point) not yet supported by count_diagrams
    ...
    t = 0.0
    try
        t = @elapsed got = count_diagrams(m, c.in_fields, c.out_fields; loops=c.loops, onepi)
    catch e
