...
exp = FeynfeldX._expand_model_for_diagen(m)
ext_exp = FeynfeldX._expand_external_fields(vcat(in_f, out_f), exp)
dpntro = build_dpntro(exp.vertex_rules)

n_topo = 0
total = 0
for dp in FeynfeldX._degree_partitions(n_ext, loops,
            Set(length(k) for k in keys(FeynfeldX.feynman_rules(m).vertices)))
